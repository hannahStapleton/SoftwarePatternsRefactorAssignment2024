...
public class EmployeeDetails extends JFrame implements ActionListener, ItemListener, DocumentListener, WindowListener {
    ...
    private static final EmployeeDetails frame = new EmployeeDetails(manager);
    ...
    public EmployeeDetails(EmployeeManager manager) {
    ...
    private void displayEmployee(Employee employee) {
        if (employee != null) {
            // Display employee details in UI components
            ...
            // Clear UI components if no employee found
            idField.setText("");
            ppsField.setText("");
    ...
    // Event handler for searching employee by ID
    private void searchEmployeeById() {
        try {
            int idToSearch = Integer.parseInt(searchByIdField.getText().trim());
            Employee foundEmployee = manager.searchEmployeeById(idToSearch);
            displayEmployee(foundEmployee); // Display the found employee
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(null, "Wrong ID format!");
        }
    }

    // Event handler for searching employee by surname
    void searchEmployeeBySurname() {
        String surnameToSearch = searchBySurnameField.getText().trim();
        if (!surnameToSearch.isEmpty()) {
            EmployeeManager manager = new EmployeeManager(); // Instantiate the EmployeeManager
            Employee foundEmployee = manager.searchEmployeesBySurname(surnameToSearch);
            if (foundEmployee != null) {
                displayEmployee(foundEmployee); // Display the details of the found employee
            } else {
                JOptionPane.showMessageDialog(null, "Employee with surname " + surnameToSearch + " not found!");
            }
        } else {
    ...
    private void displayEmployeeSummaryDialog() {
        // display Employee summary dialog if this is someone to display
    ...
    private void deleteRecord() {
        if (isSomeoneToDisplay()) {// if any active record in file display
            ...
            if (returnVal == JOptionPane.YES_OPTION) {
                boolean recordDeleted = manager.deleteRecord();
                if (recordDeleted) {
                    if (isSomeoneToDisplay()) {
                        nextRecord();// look for next record
    ...
    public boolean validatePps(String pps, long currentByteStart) {
        return manager.correctPps(pps, currentByteStart);
    }

    // check if file n
    // ame has extension .dat
    ...
    private boolean checkInput() {
        ...
        if (ppsField.isEditable() && validatePps(ppsField.getText().trim(), currentByteStart)) {
            ppsField.setBackground(new Color(255, 150, 150));
            valid = false;
        } // end if
